...
function Psk() {
  ...
  const modulationIndex = Math.PI; // BPSK
  ...
  const generatePSKSignal = () => {
    ...
    for (let i = 0; i < binaryData.length; i++) {
      ...
      for (let j = 0; j < samplesPerBit; j++) {
        const sample = bit === 0 ? 1 * Math.cos(2 * Math.PI * carrierFrequency * t) : -1 * Math.cos(2 * Math.PI * carrierFrequency * t);
        PSKsignal.push(sample);
        t += 1 / carrierFrequency / samplesPerBit;
      }
    }
    setModulatedSignal(PSKsignal);
  };
  ...
  return (
    ...
    <div className="p-4 mx-auto mb-24">
      <h2 className="text-xl font-semibold mb-4 text-center">BPSK Modulation Simulation</h2>

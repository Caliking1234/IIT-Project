...
const page = () => {
  ...
  useEffect(() => {
    ...
    const demodulateFM = (fmSignal, carrierFrequency, modulationIndex, signalDuration) => {
      // const stepSize = 1 / 1000; // 1000 steps per second
      ...
      for (let i = 0; i < fmSignal.length; i++) {
        ...
        const deltaPhase = 2 * Math.PI * (fmSignal[i] - carrierFrequency) / carrierFrequency;
  ...
  // return demodulatedSignal;
    ...
    let QuantizeSignal = [];
    QuantizeSignal = quantizeSignal(DemodSignal, 1.9);
    ...
    setFmSignal(fmSignalData);
    setDmodfmSignal(demodulatedfmsigdata);
    console.log(demodulatedfmsigdata);

  }, [amplitude, carrierFrequency, modulationFrequency, signalDuration]);

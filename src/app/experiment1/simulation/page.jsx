...
const page = () => {
  ...
  const [TpamSignal, setTPamSignal] = useState([]);

  const [PWM, setPWM] = useState([]);
  const [pwmSignal, setPwmSignal] = useState([]);
  const [TpwmSignal, setTPwmSignal] = useState([]);
  ...
  useEffect(() => {
    ...
    //Trinangle wave generate hori hai
    ...
    const TriangleSignalData = generateTriangleWave(signalDuration, frequency);

    const pamSignalData = carrierSignalData.map(
      (carrierValue, i) => carrierValue * pulseSignalData[i]
    );
    const pamSignalTPulseData = carrierSignalData.map(
      (carrierValue, i) => carrierValue * TriangleSignalData[i]
    );

    setTPamSignal(pamSignalTPulseData);
    setPamSignal(pamSignalData);

    setTime(timeAxis);
    setCarrierSignal(carrierSignalData);
    setPulse(pulseSignalData);
    setTPulse(TriangleSignalData);
    ...
    console.log(pwmSignalData);

    setPwmSignal(pwmSignalData);

    const TPWMDATA = carrierSignal.map((carr, i) =>
      carr > TriangleSignalData[i] ? 1 : 0
    );

    setTPwmSignal(TPWMDATA);
    console.log(TPWMDATA);
  }, [
    amplitude,
    signalDuration,
    carrierFrequency,
        ...
        <div className="w-full flex flex-col gap-5 items-center justify-center shadow-md shadow-black bg-white p-4">
          ...
          {type == 1 || 2 ? (
            <div className=" w-full flex gap-2 px-5">
              ...
              >
                <option selected>Choose Simulation</option>
                <option value="1">Square Pulse</option>
                <option value="2">Triangle Pulse</option>
              </select>
            </div>
          ) : (
            <></>
